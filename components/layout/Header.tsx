...
function MegaMenuDropdown({ data, isOpen }: { data: typeof menuData.services; isOpen: boolean }) {
    return (
        <AnimatePresence>
            {isOpen && (
                <motion.div
            ...
            <div className="container mx-auto px-4 flex items-center justify-between">
                ...
                <nav className="hidden lg:flex items-center gap-1">
                    {menuKeys.map((key) => (
                        ...
                        >
                            <button className="flex items-center gap-1 px-4 py-2 text-sm font-medium text-slate-700 hover:text-blue-600 transition-colors rounded-lg hover:bg-slate-50">
                                {menuData[key].title}
                                <ChevronDown size={14} className={cn("transition-transform", activeMenu === key && "rotate-180")} />
                            </button>
                            <MegaMenuDropdown data={menuData[key]} isOpen={activeMenu === key} />
                        </div>
                    ))}
                    <Link href="/pricing" className="px-4 py-2 text-sm font-medium text-slate-700 hover:text-blue-600 transition-colors rounded-lg hover:bg-slate-50">
                        Pricing
                    </Link>
                </nav>
                        ...
                        <div className="container mx-auto px-4 py-6 space-y-6">
                            {menuKeys.map((key) => (
                                <div key={key} className="space-y-2">
                                    <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">{menuData[key].title}</div>
                                    {menuData[key].items.map((item, index) => (
                                        <Link key={index} href={item.href} className="block py-2 text-slate-700 hover:text-blue-600" onClick={() => setIsOpen(false)}>
                                            {item.name}
                                        </Link>
                                    ))}
                                </div>
